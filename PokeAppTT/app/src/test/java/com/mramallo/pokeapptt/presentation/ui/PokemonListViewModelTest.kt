...
class PokemonListViewModelTest {
    ...
    @Test
    fun `getPokemonList when success should return list of Pokemons`() = runTest {
        val pokemonListExpectedData: PagingData<PokemonResult> =
            PagingData.from(listOf(testPokemonResult))
        val flowPokemon = flowOf(pokemonListExpectedData)

        coEvery {
            getPokemonListUseCase.invoke().flow
        } returns flowPokemon
        ...
        runBlocking {
            ...
            assert(collectedData[0] == pokemonListExpectedData)
        }
    ...
    fun `getPokemonElement when success should return detail of pokemon using uuid and name`() =
        runTest {
            val pokemonElementExpectedData: PokemonDetail = testPokemonDetail

            coEvery {
                getPokemonDetailUseCase.invoke("Pikachu")
            } returns pokemonElementExpectedData

            pokemonListViewModel.getPokemonElement(testPokemonResult.uuid, testPokemonResult.name)

            assertEquals(
                PokemonListViewModel.PokemonElementState(pokemonDetail = pokemonElementExpectedData, loading = false),
